...
import { CHANGEPASSWORDVERIFY_URL, RESENDOTP_URL, VERIFYACCOUNT_URL } from "../../Utils/ApiUrlConstants";
...
export default function ChangePasswordOTP({ navigation,route }) {
  ...
  //verify api params
  const verifyUrl = CHANGEPASSWORDVERIFY_URL;
  let signInParams = {
    otp:OTP?.trim(),
  };
  ...
  const validationHandler = () => {
    ...
    if (valMsg) {
      ...
      setLoader(true);
      fetchUrl(signInParams, token, verifyUrl)
        .then((resJson) => {
          ////////////////////
          setLoader(false);
      ...
      >
        <View style={styles.ViewIntoScrollView}>
          ...
          <Text style={styles.subTitle}>
            A 4 digit code has been sent to you registered email address {userEmail}
          </Text>
