...
import { RESENDOTP_URL, VERIFYACCOUNT_URL } from "../../Utils/ApiUrlConstants";
...
export default function VerifyAccount({ navigation,route }) {
  ...
  //verify api params
  const verifyUrl = VERIFYACCOUNT_URL;
  let signInParams = {
    email: userEmail, 
    otp:OTP?.trim(),
  ...
  const validationHandler = () => {
    ...
    if (valMsg) {
      ...
      setLoader(true);
      fetchUrl(signInParams, "", verifyUrl)
        .then((resJson) => {
          ////////////////////
          setLoader(false);
          ...
          if (resJson?.status == 200) {
            // console.log("accessToken", resJson?.accesss_token);
            dispatch(setVerifyEmailAction(resJson?.data?.email));
            dispatch(setTokenAction(resJson?.accesss_token)); //
            let tokenStored = setItemInStorage("accessToken", resJson?.accesss_token); //
